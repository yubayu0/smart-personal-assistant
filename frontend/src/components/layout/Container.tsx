...
const Container = ({ children }: { children: React.ReactNode }) => {
  ...
  const handleAlertCardClose = () => {
    setAlertCardOpen(false)
    document.body.style.overflow = "auto"
  }

  return (
    <div className='w-full flex flex-col justify-center items-center bg-grid-lg'>
      <div className='w-full flex flex-col h-40 p-4 justify-center items-center '>
        <nav className='rounded-md h-auto fixed top-4 min-w-[500px] md:min-w-[800px] z-99 flex flex-col bg-white neu'>
          <div className='flex justify-between items-center  w-full px-4 py-4 md:px-8 gap-8'>
          ...
          <div className={`block md:hidden w-full   px-4 overflow-hidden  justify-items-center transition-all duration-500 ease-in-out ${open ? "max-h-[400px] opacity-100 pb-4" : "max-h-0 opacity-0"
            }`} >
            <div className='flex flex-col justify-center items-start gap-3 font-bold w-full border-t-4 border-black pt-4' >
              {menu.map((menu) => (
                <button
                  className='flex justify-start items-center gap-2 hover:bg-gray-400 w-full'
                  key={menu.path}
                  onClick={() => handleScroll(menu.path)}
      ...
      <footer className='w-full mt-20 flex justify-between items-center p-2'>
      ...
      {alertCardOpen ? (
        <div className='absolute inset-0   z-99 w-full h-full flex justify-center items-center'>
          <AlertCard
            headMessage='Hello'
            message='Yohooo'
            message2='kamu berhasil login'
            buttonText='Clik Me'
            actionClick={() => window.open('https://github.com/', '_blank')}
            closeCard={() => handleAlertCardClose()
            }
          />
        </div>
      ) : (
